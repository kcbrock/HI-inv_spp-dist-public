---
title: " - Preparing local Model Inputs"
author: "Kelsey Brock"
...
```
```{r}
# select only herbarium specimens if desired
...
```{r}
pres2 <- read.csv(paste(path.in.general, "/occurrence_data/", "local_herbarium_occurrences.csv", sep = ""), header=T, sep=',', stringsAsFactors=F)

pres2 <- subset(pres2, scientificName == "Species name")
pres2 <- subset(pres2, !is.na(decimalLatitude))
pres2 <- subset(pres2, !is.na(decimalLongitude))
pres2
```
```{r}
unique(pres2$scientificName)
```

```{r}
pres2$scientificName <- gsub("Species name", 1, pres2$scientificName)

pres2 <- dplyr::rename(pres2, Species = scientificName ) %>% dplyr::select(Species, decimalLatitude, decimalLongitude)
head(pres2, 2)
```

```{r}
pres <- pres %>% dplyr::select(Species, decimalLatitude, decimalLongitude)
...
```{r}
output <- spThin::thin(pres, lat.col = 'decimalLatitude', long.col = 'decimalLongitude', spec.col = "Species", thin.par = 1.5, reps = 3, locs.thinned.list.return = TRUE, write.files = FALSE, verbose = TRUE)
```
```{r}
beep(sound = 2)
```
...
write.csv(pres, paste0(spname, "_occurrences_used_local.csv"), row.names = FALSE)
```
...
```
```{r}
st_crs(local.bufbboxSF)$proj4string == st_crs(local.bufptSF)$proj4string
```
```{r}
...
```{r}
local.bufptSP <- as_Spatial(local.bufptSF) # converts to spatial polygon class
local.bufptR <- crop(rasterize(local.bufptSP, template1), local.bufptSP) # cropping to bbox
local.bufptR
```
```{r}
plot(local.bufptR, legend = T)
```
#### Save
```{r}
setwd(path.in.specific)
writeRaster(local.bufptR, paste0(spname, "_local.bboxR.img"), format = "HFA", overwrite = TRUE)
```
...
pres.fnetid <- cellFromXY(local.bufptR, p1) # FNETID of presence from pt-buffered poly
length(pres.fnetid)
...
```{r}
bufpt.fnetid <- raster::extract(local.bufptR, local.fnetDF[c("decimalLongitude", "decimalLatitude")])
...
```{r}
local.bufptFNET <- cbind(local.fnetDF, bufpt.fnetid) # bind modelling frame w/FISHNET
head(local.bufptFNET, 2) # examine
```


```{r}
plot(local.bufboxR, legend = T)
plot(local.bufptR, col = "red", add = T, legend = F, main = "Points in modelling frame")  # main plot
...
m1 <- merge(local.fnetDF, local.bufptFNET, by = c("FNETID", "decimalLongitude", "decimalLatitude" ), all.y = T)
...
```{r}
local.PPsA <- merge(local.presFNET, pseu.srs2, by = c("FNETID", "Species", "decimalLongitude", "decimalLatitude"), all = T) # merge
...
local.PPsA$tr.Lon <- ifelse(local.PPsA$Species == 0, local.PPsA$decimalLongitude, local.PPsA$decimalLongitude)
local.PPsA$tr.Lat <- ifelse(local.PPsA$Species == 0, local.PPsA$decimalLatitude, local.PPsA$decimalLatitude) 
...
Using HI-specific climate data

```{r}
setwd(path.preds)
baseline <- list.files(pattern = "baseline.tif") # list of .img files; $ strips extra
...
writeRaster(baseline_stack, "HI_bioclimstack", format = "HFA", overwrite = T) # save as .img file
```
```{r}
setwd(path.in.general)
bioclim <- get(load("HI_bioclimstack.RData"))
bioclim
```
...
```{r}
# examine extractions
nrow(subset(local.trTOPO, is.na(as.character(local.trTOPO$bio1))))
```
...
```{r}
local.trTOPOSF <- st_as_sf(local.trTOPO, coords = c("tr.Lon", "tr.Lat"), crs = prj.wgs84, remove = FALSE)
class(local.trTOPOSF)
```


```{r}
zeros <- subset(local.trTOPOSF, Species == 0)
ones <- subset(local.trTOPOSF, Species == 1)
...
tr.data <- subset(tr.data, !is.na(tr.data$annual_precip_baseline))
dim(tr.data)
```
```{r}
...
tempvars <- c('annual_tmax_baseline', 'annual_tmean_baseline', 'annual_tmin_baseline', 'drySeason_tmax_baseline', 'drySeason_tmean_baseline', 'drySeason_tmin_baseline', 'wetSeason_tmax_baseline', 'wetSeason_tmean_baseline', 'wetSeason_tmin_baseline')
precipvars <- c('annual_precip_baseline', 'drySeason_precip_baseline', 'wetSeason_precip_baseline')
...
```{r}
keep <- c("annual_tmax_baseline", "annual_tmean_baseline", "annual_precip_baseline", "drySeason_tmax_baseline",   "drySeason_tmean_baseline" , "drySeason_precip_baseline" ,"wetSeason_precip_baseline")

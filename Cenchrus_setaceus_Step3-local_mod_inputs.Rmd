---
title: "Cenchrus setaceus - Preparing local Model Inputs"
author: "Kelsey Brock"
...
```
```{r}
pres$Species <- gsub(spname, 1, pres$Species)
head(pres, 2)
```
...
pres2$scientificName <- sub(" ", "_", pres2$scientificName )
pres2 <- subset(pres2, scientificName == spname)
...
```{r}
pres2$scientificName <- gsub(spname, 1, pres2$scientificName)
...
```{r}
kms_to_thin_by = 1.5
output <- spThin::thin(pres, lat.col = 'decimalLatitude', long.col = 'decimalLongitude', spec.col = "Species", thin.par = kms_to_thin_by, reps = 1, locs.thinned.list.return = TRUE, write.files = FALSE, verbose = TRUE)
...
```
```{r}
table(local.indexFNET$Species)[[1]] # does it match w/above ??
```
```{r}
...
pseu.srs2 <- pseu.islandpoly[sample(1:nrow(pseu.islandpoly), 3.5 * table(p1$Species)[[1]], replace = F), ]
...
```{r}
baseline_stack <- c(baseline, elevation, seasonal_stack)
baseline_stack
```
...
```{r}
names(baseline_stack) <- c("bio1",  "bio2",  "bio3",  "bio4",  "bio5",  "bio6",  "bio7",  "bio8",  "bio9", "bio10", "bio11", "bio12", "bio13", "bio14", "bio15", "bio16", "bio17", "bio18", "bio19", "alt", "bio21", "bio22", "bio23", "bio24", "bio25", "bio26", "bio27", "bio28"  )
...
```{r}
keep <- c("bio2", "bio4", "bio7","bio18", "bio21")
